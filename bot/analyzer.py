...
def seo_summarize(free_body: str, home: str, away: str, league: str,
    ...
    system = (
        ...
        "列表展示的文案。只输出一个 JSON 对象，不要 markdown 代码块、不要多余文字。字段：\n"
        '  "hook": 一句话看点（≤20字），勾起读者兴趣，用作标题副标题；\n'
        '  "excerpt": 文章摘要（160~240字，可分两小段），用【基本面故事】写：'
        "先讲两队近况战绩（近 10 场胜平负、进失球、本赛事/淘汰赛表现、状态与攻防特征、"
        "对手质量），再讲历史交锋与赛事情境（中立场/主客场、是否决赛/淘汰赛、加时点球规则、"
        "休息与赛程恢复天数等看点）。像给读者做赛前背景导读，客观、可读、有信息量；\n"
        '  "meta_desc": 搜索引擎描述（≤150字），取 excerpt 前半段的核心，'
        "把队名与赛事关键词前置。\n"
        "硬规则：① 可以写两队【过去】的战绩/进球/交锋/状态等客观事实（这些是背景，不是预测），"
        "但【绝不】泄露本场的比分预测、胜平负/上下盘/大小球等结论性判断；"
        "② 不出现任何具体庄家名（如 365/Pinnacle/威廉），也不提「盘口/欧赔/亚盘/凯利/"
        "资金流向/返还率」这类操盘术语——摘要面向普通球迷讲基本面，不讲赔率方法论；"
        "③ 全中文；④ 视角是【{view}】；⑤ 只依据给定正文，不编造数据，正文没有的战绩/"
        "交锋不要杜撰；⑥ 不罗列盘口数字。"
    ).replace("{view}", view)
    user = (
        f"## 比赛：{home} vs {away}（{league}）\n"
    ...
    return {"hook": hook[:30], "excerpt": excerpt[:300],
            "meta_desc": meta_desc[:150]}, None
