...
def format_states_block(st: dict | None) -> str:
    ...
    if not st:
        return ("### 进球分布模型\n"
                "（大小球盘口缺失或不可用，无法估 λ）\n"
                "- **仅 |让球| ≥0.75 的深盘**本场标注「无法估 edge」，"
                "只用 p_市场 参考、不下注。\n"
                "- **|让球| ≤0.5（含平手、±0.25、±0.5）不受影响**：其五态由胜平负概率"
                "完全确定，不需要 λ。必须照常算出数字 edge——\n"
                "  受让 +0.25：`edge = p_胜×(O−1) + p_平×0.5×(O−1) − p_负`；"
                "让 −0.25：`edge = p_胜×(O−1) − p_平×0.5 − p_负`。\n"
                "- edge 列**不得**写「无法确认/需精确核价/数据口径不足」等文字替代数字。\n")
    lines = [
