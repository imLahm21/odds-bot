...
## 证据分层与优先级（先读这段，再读下面任何规则）

**四层证据，方向与置信度分两套机制处理——不可混用：**

| 层 | 内容 | 来源性质 | 权重 | 能否改「方向」 |
|---|------|---------|------|--------------|
| **A** | 盘口定性（步骤 2）+ 操盘手法匹配（步骤 3、4 军规/诱盘） | 【数据类】API 抓取 | **40%** | **定方向**（唯一方向来源） |
| **C** | 风控指数（凯利、返还率） | 【数据类】API 抓取 | **20%** | **不可改方向** |
| **B** | 基本面（伤停/战意/赛程/近况/H2H 数据本身） | 【文字类】手工粘贴 | **30%** | 仅「条件覆盖」场景可改（见下） |
| **D** | 案例教训（本目录 `feedback_*.md`） | 【文字类】事后归纳 | **10%** | **不可改方向** |

> **两种分组各说明一件事，别混为一谈**：
> · **数据类（A+C，60%）vs 文字类（B+D，40%）** 说的是**可信度来源**——
>   数据类由 API 客观抓取、可复核、无主观加工；文字类依赖粘贴完整度与归纳准确性。
> · **A/B/C/D 层级** 说的是**方向权限**——与上面的分组无关。
> 于是有个看似矛盾却成立的安排：C（20%，数据类）**不可**改方向，
> B（30%，文字类）在条件覆盖下**可以**改。原因是 **B 是独立信息源，C 不是**——
> C 由同一份盘口数据算出，不构成对 A 的独立验证。

### 方向怎么定（层级决定，不按权重投票）

1. **方向由 A 层决定**。A 是一手信息——读的是庄家的定价行为本身（开多深、往哪调、水位如何配合）。
   A 层内部若冲突（定性与变盘路径矛盾），**以变盘路径（算法 B）为准**。
2. **B 层只在明确触发的「条件覆盖」场景可改方向**，且须写明触发条件：
   碾压级基本面（五要素）、极端动力差异（保级死战 vs 无欲无求 ≥3 项）等。
   常规矛盾场景下 B 层不改方向，仅按权重扣置信度。
3. **C、D 层永不改方向**，只做两件事：**扣置信度** + **把反向结果列为次选/备选**。
   · C 层（凯利/返还率）是盘口的衍生量——同一个 0.92 可能是看好某方，也可能是该庄抽水高、
     挂盘慢、或在对冲持仓，信息含量低于 A 层一个量级。
   · D 层是事后归纳，与 A 同源，冲突时更可能是自己算错而非发现了新信息。

> ⚠️ **不可按权重加总投票决定方向**。若按 B+C+D=60% > A=40% 去投票，
> C 与 D 联手即可推翻 A，与上面第 3 条直接矛盾。**权重只用于算置信度。**

### 置信度怎么算（权重决定）

四层一致 → 高置信度；某层与 A 冲突 → 按该层权重扣减：

| 冲突层 | 置信度扣减 |
|--------|-----------|
| B 层与 A 冲突（非覆盖场景） | 扣 10~15 分 |
| C 层与 A 冲突 | 扣 5~10 分 |
| D 层与 A 冲突 | 扣 3~5 分 |

多层同时冲突则累加；A 层内部冲突（定性 vs 变盘）额外扣 10 分并封顶 65。
...
4. 实战教训属于**D 层（权重 10%）**：只扣置信度、增列备选，**不改方向**（见本文开头「证据分层与优先级」）。基本面类的「条件覆盖」场景（碾压级基本面、保级死战等）属 B 层，须满足明文触发条件才可覆盖 A 层方向。
...
| 5 | 任一公司凯利极端值（>1.05 / <0.95）、方向反转、或初盘开小（浅盘）读凯利 | [feedback_kelly_signals.md](feedback_kelly_signals.md) | Pinnacle 双重极端=最强扣分项（不改方向）；凯利反转方向>绝对值；浅盘灰色地带；平局凯利信号 |
| 6 | 中超赛事 | [feedback_csl_fundamentals.md](feedback_csl_fundamentals.md) | 主场反弹潜力；单败非崩溃；「排名差+主场不败」上限 25%；保级死战关闭凯利反转；置信度上限 75 |
| 7 | 有 H2H 交锋数据，或两队积分差 ≤3 分 | [feedback_h2h_weight.md](feedback_h2h_weight.md) | 平局率 ≥60% → 平局并列首选（不改方向）；H2H 权重随本赛季实力差双向调整（35% ↔ 20%） |
...
- [ ] **H2H 平局率 ≥ 60%** → 平局**并列首选** + 扣 5~10 分（D 层不改方向），不可仅列风险提示（→ 主题 7）
...
> **机器人自动归档**：完整可执行步骤见 [ARCHIVING_PROTOCOL.md](ARCHIVING_PROTOCOL.md)（前置编号 → 路由 → 三写一改 → 自检 → **自动 commit**）。**机器人到 commit 为止，push 由人工审阅后执行**——「归档成功」不等于「已推送」。下列为要点摘录。
