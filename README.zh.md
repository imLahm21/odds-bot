...
| 任务 | 频率 | 动作 |
|------|------|------|
| A | 每日 02:00 / 14:00（北京时间） | 拉关注联赛完整赛季赛程 → `fixtures`（可纠正移出短期窗口的占位时间） |
...
| `/parlay <id1> <id2> <id3>` | **（Beta）** 3串1 串关分析：现场跑 3 场单场精算 → 确定性裁判（每腿 edge>0 且证据≥中才可串）|
...
> 然后跑重档模型（流式，实时播报进度）并归档到 `report/<年>/<月>/<日期>/`。
> **`/review`** 与 `/analyze` 完全独立：先不告诉 AI 比分、只给盘口走势正向推一遍，再揭晓真实比分做对照，
> 报告归档为 `report/<年>/<月>/<日期>/<主队>_vs_<客队>_review.md`。
...
`gpt-6-astra` 合并报告、`gpt-5.6-sol` 审计。每个模块配置同能力等级、跨授权组的回退链，
并为每个回退模型使用其独立的推理强度和 token 预算；流式合并只在首字节前转移。
会诊报告使用 `_consult.md` 后缀归档，不覆盖旧版 `_report.md`。报告末尾的「模型会诊记录」
会显示请求模型、实际模型和故障转移尝试。

**全模型交叉复盘**：管理员在 `/review` 中可选择 `🧪 全模型交叉复盘`，并选择标准或
自定义侧重。第一阶段由全模型在看不到比分和基本面的前提下盲推；盲推完成后才揭晓赛果、
拉取基本面，由全部模型分模块归因，最后仍由合并模型链合并、审计模型链审计。合并档使用
`_review_consult.md` 后缀，不覆盖旧版 `_review.md`。
...
#### 🎰 `/parlay <编号1> <编号2> <编号3>` — 3串1 串关分析（Beta）

对**三场不同的**未开赛比赛做串关判断（如 `/parlay 158726 123548 223894`，空格分隔、无按钮）。
bot 会**现场把三场各跑一遍单场精算**，逐腿抽出 edge，再做一次确定性串关裁判：
**每腿 edge>0 且证据达「中」以上**才判「可串」并给合并 EV + 建议注额（BCG 3串1 享 ×1.15 增益）；
任一腿不达标就判「拆单关」。跑完你会收到 3 份单场报告 + 1 份串关结论。

> ⚠️ **Beta 实验功能**，串关判定仍在打磨、结果仅供参考。
> **配额**：串关每天最多 **2 次**；每跑一次**扣 3 次**精算额度（与 `/analyze` 共享每天 10 次的额度池）。
> 只做 3串1（2串1 无 ×1.15 增益，数学上不划算）。三腿必须是不同比赛。
